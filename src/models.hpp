...
struct PaginationInfo
{
    ...
    PaginationInfo(int page = 1, int per_page = 20) : page(page), per_page(per_page), total_items(0), total_pages(0)
    {
    }
};
...
struct DataLocation
{
    ...
    DataLocation(const std::string &repo, const std::string &json_path, const std::string &schema_path,
                 const std::string &fallback_hash)
        : repository(repo), json_path(json_path), schema_path(schema_path), fallback_hash(fallback_hash)
    {
    }
};
...
struct Link
{
    ...
    Link(const std::string &name, const std::string &url) : name(name), url(url)
    {
    }
};
...
struct Repository
{
    ...
    Repository(const std::string &name, const std::string &url) : name(name), url(url)
    {
    }
};
...
struct Collaborator
{
    ...
    Collaborator(const std::string &username, const std::string &url) : username(username), url(url)
    {
    }
};
...
struct Category
{
    ...
    Category(const std::string &key, const std::string &name, const std::string &description, const std::string &emoji)
...
struct Showcase
{
    ...
    Showcase(const std::string &title, const std::string &description, const std::string &link)
